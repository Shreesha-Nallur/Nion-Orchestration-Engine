"""OutputRenderer: formats the final orchestration map report exactly per
the spec's expected output format - header, L1 PLAN section, L2/L3
EXECUTION section, footer.
"""
...
def _format_output_block(output_lines: List[str], indent: str) -> List[str]:
    """Render a list of agent output lines as bullet points. Lines that
    themselves contain embedded newlines (e.g. a multi-paragraph qna
    response) are split so each physical line still lines up with `indent`,
    but only the first line of a multi-line block gets the bullet."""
    rendered = []
